    ...
    for _layer in range(all_Q.shape[0]):
        ...
        for _head in range(head//head):
            ...
            for _sta_k in range(global_tokens): 
                ...
                weight_cycles += my_SRAM.preload_K(nums=head*args.ratio*1* K.shape[1], bits=8, bandwidth_ratio=1)
                if _sta_k == 0:
                    preload_cycles += my_SRAM.preload_decoder(nums=head*args.ratio*1, bits=8, bandwidth_ratio=1/head)
                    weight_cycles += my_SRAM.preload_decoder(nums=head*args.ratio*1, bits=8, bandwidth_ratio=1/head)
                # ######### Preprocessing 
                ...
                for _sta_q in range(int(Q.shape[0])):
                    ...
                    # ######### Load q and decoder weight
                        ...
                        input_cycles += my_SRAM.preload_Q(nums=head*args.ratio*1* Q.shape[1], bits=8, bandwidth_ratio=1)*(1+reload_ratio)
                        if _sta_q == 0: 
                            preload_cycles += my_SRAM.preload_decoder(nums=head*args.ratio*1, bits=8, bandwidth_ratio=1/head)
                            input_cycles += my_SRAM.preload_decoder(nums=head*args.ratio*1, bits=8, bandwidth_ratio=1/head)
                        # ######### Preprocessing 
